...
class Subscription extends MY_Controller {
	...
	public function save(){
		if ($this->checkLogin('A') == ''){
			...
			$array = ['event_listing', 'registration', 'live_score', 'featured_tag', 'cover_image', 'photo_gallery', 'video_gallery', 'sm', 'rj', 'vj'];
			foreach($array as $arr){
				$_POST[$arr] = (array_key_exists($arr, $_POST))?'1':'0';
			}
			...
			if($objectid!=''){
				...
				if ($adminCheck > 0){
					$this->admin_model->commonInsertUpdate(SUBSCRIPTION, 'update', ['_id', 'objectid', 'status'], ['dateModified'=>(int)time()], $conditionArr);
				...
				$this->admin_model->commonInsertUpdate(SUBSCRIPTION,'insert',['_id', 'objectid', 'status'],['dateAdded'=>MongoDATE(time()), 'status'=>'1'],[]);
				$this->setErrorMessage('success','Subscription details saved!!!');
				redirect(ADMINURL.'/subscription-list');
			}
		}
