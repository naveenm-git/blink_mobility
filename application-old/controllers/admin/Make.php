		...
		if ($this->checkLogin('A') == ''){
			...
			if($objectid!=''){
				...
				if ($adminCheck > 0){
					...
					$this->admin_model->commonInsertUpdate(MAKE, 'update', ['_id', 'objectid', 'status', 'password', 'dob', 'subscription_date', 'activation_date', 'expiry_date'], $dataArr, $conditionArr);
				...
				$password = $this->input->post('password');
				if($password!='')	$dataArr['password'] = md5(trim($password));
				$this->admin_model->commonInsertUpdate(MAKE,'insert',['_id', 'objectid', 'status', 'password', 'dob', 'subscription_date', 'activation_date', 'expiry_date'], $dataArr,[]);
				$this->setErrorMessage('success','Make details saved!!!');
				redirect(ADMINURL.'/make-list');
			}
		}
