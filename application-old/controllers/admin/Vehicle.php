...
class Vehicle extends MY_Controller {
	...
	public function save(){
		if ($this->checkLogin('A') == ''){
         ...
         $condition = ['vin_number'=>$vin_number];
         if($objectid!='') $condition['_id'] = ['$ne'=>objectid((string)$objectid)];
         ...
         if($getDuplicate == 0){
            ...
            $this->setErrorMessage('error','VIN number already exist!!!');
         }
         redirect(ADMINURL.'/vehicle-list');
		}
	}
   ...
   public function get_maker_models(){
      ...
      $make = $this->admin_model->get_all_counts(MAKE, ['_id'=>objectid($make_id), 'status'=>'1']);
      if($make > 0){
