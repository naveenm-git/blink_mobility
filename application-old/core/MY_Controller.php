...
class MY_Controller extends CI_Controller {
    ...
    function __construct() {
         ...
         $this->data['validityInterval'] = ['days'=>'Day(s)', 'weeks'=>'Week(s)', 'months'=>'Month(s)', 'years'=>'Year(s)'];
	...
	**/
	...
	public function subscriber_email_template($emailArr, $template){
		extract($emailArr);
		$base_url = base_url();
		$template_details = $this->admin_model->get_all_details(NEWSLETTER, ['_id'=>objectid($template)]);
		$message = $template_details->row()->content;
		$message = str_replace(["{","}"],"",addslashes($message));
		eval("\$message = \"$message\";");
		$email_values = array (
...
Class Api_Controller extends MY_Controller {
		...
		function __construct(){
			...
			$this->_open_requests = array('sample_notification', 'index', 'app_info', 'home_page', 'search', 'categories', 'sub_categories', 'proposal', 'register', 'login', 'forgot_password', 'get_pages', 'page','send_bulk_notification','send_inbox_notification','delivery_times','send_single_notification','custom_test');
