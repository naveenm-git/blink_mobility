      ...
      <div class="col-md-12 col-xs-12">
				<div class="box-content bordered default">
					...
					<?php $attributes = array('class' => 'form-horizontal form-swal-submit', 'method' => 'POST', 'enctype' => 'multipart/form-data', 'data-toggle' => 'validator'); echo form_open('admin/station/save',$attributes); ?>
						...
						<div class="form-group">
							<label class="col-sm-3 control-label" for="parkings">Parking <span class="req">*</span></label>
							<div class="col-sm-9">
                        <select name="parkings" id="parkings" class="form-control" required>
                           <option value="">Select no.of parking</option>
                           <?php for($park=1;$park<=5;$park++){ ?>
                              <option value="<?php echo $park; ?>" <?php echo (count($result)>0)?(($result['parkings']==$park)?'selected':''):''; ?>><?php echo $park.' '.(($park==1)?'Parking':'Parkings'); ?></option>
                           <?php } ?>
                        </select>
								<div class="help-block with-errors"></div>
							</div>
                     ...
                     <div class="col-sm-9">
                        <select name="vehicles[]" id="vehicles" class="form-control select2-multiple" multiple="multiple" required>
                           <?php foreach($vehicles->result() as $vehicle){ ?>
                              <option value="<?php echo (string) $vehicle->_id; ?>" <?php echo (count($result)>0 && isset($result['vehicles']) && count($result['vehicles']) && in_array((string) $vehicle->_id, $result['vehicles']))?'selected="selected"':''; ?>><?php echo $makes[$vehicle->make_id].' '.$models[$vehicle->model_id].' '.$vehicle->year; ?></option>
                           <?php } ?>
                        </select>
                     </div>
                  </div>
